...
class InitDBCommandTestCase(TestCase):
    ...
    def test_load_revisions(self):
        ...
        self.assertEqual(Revision.objects.filter(type='b').count(), 16)
        self.assertEqual(Revision.objects.all().count(), 181)
    ...
    def test_map_cves_to_revisions(self):
        ...
        for revision in self.cves_count:
            self.assertEqual(
                CveRevision.objects.filter(revision__number=revision,
                    revision__type='t').count(),
                self.cves_count[revision], msg='%s' % revision
            )

        for item in self.cve_fix_commit_hashes:
            self.assertEqual(
